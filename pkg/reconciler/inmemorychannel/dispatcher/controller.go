...
func NewController(
	...
	// Nothing to filer on, enqueue all imcs if configmap updates.
	noopFilter := func(interface{}) bool { return true }
	resync := configmap.TypeFilter(channel.EventDispatcherConfig{})(func(string, interface{}) {
		r.impl.FilteredGlobalResync(noopFilter, informer)
	})
	// Watch for configmap changes and trigger imc reconcilation by enqueuing imcs.
	configStore := channel.NewEventDispatcherConfigStore(base.Logger, resync)
